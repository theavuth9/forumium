<div class="container mx-auto flex items-center flex-wrap">
    @if($tags->count())
        @foreach($tags as $tag)
            <a href="{{ route('tag', ['tag' => $tag, 'slug' => $tag->slug]) }}" class="lg:w-1/3 w-full p-6 text-white hover:shadow-xl shadow-slate-900 hover:scale-105 transition-all hover:rounded-xl" style="background-color: {{ $tag->color }};">
